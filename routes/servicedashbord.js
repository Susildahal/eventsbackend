...
servicedashbordRouter.post("/", upload.any(), verifyToken, createVenueSourcing);
// Route to update an existing venue sourcing entry by ID (accepts any multipart file field)
servicedashbordRouter.put("/:id", upload.any(), verifyToken, updateVenueSourcing);
// Route to upload/replace only the hero image for an existing VenueSourcing
servicedashbordRouter.post("/:id/image", upload.any(), verifyToken, uploadVenueImage);
